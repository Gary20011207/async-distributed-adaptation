...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot MLLM federated logs.")
...
def _load_csv(path: Path) -> pd.DataFrame:
    ...
    if "method" not in frame.columns:
        frame["method"] = path.stem.split("_mllm_")[0]
...
def _plot_test_acc_vs_progress(data: pd.DataFrame, outdir: Path) -> None:
    ...
    plt.legend(fontsize=7, loc="upper center", bbox_to_anchor=(0.5, -0.16), ncol=2)
    plt.subplots_adjust(bottom=0.28, right=0.98, top=0.90)
...
def _new_figure() -> None:
    plt.figure(figsize=(11, 5.8), constrained_layout=False)
