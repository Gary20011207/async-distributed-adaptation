...
from fed_mllm.client import FederatedMLLMClient, evaluate_model
from fed_mllm.experiment_logging import ExperimentLogger, save_checkpoint
from fed_mllm.model import create_model, get_parameters, interpolate, set_parameters
...
def evaluate_global(
    ...
    *,
    ...
    loss, accuracy, _, invalid_rate = evaluate_model(model, testloader, device)
    del model
    if device.type == "cuda":
        torch.cuda.empty_cache()
    return loss, accuracy, invalid_rate
...
def run_sync_fedavg(
    clients: list[FederatedMLLMClient],
    testloader: DataLoader,
    device: torch.device,
    rounds: int,
    ...
    *,
    ...
    dataset_name: str = "mllm",
    num_classes: int = 4,
    input_mode: str = "text",
    model_name: str = "tiny_text",
) -> list[np.ndarray]:
    global_model = _create_model_on_device(
        device,
        num_classes=num_classes,
        input_mode=input_mode,
        model_name=model_name,
    ...
    for rnd in range(1, rounds + 1):
        ...
        final_loss, final_acc, final_invalid_rate = evaluate_global(
            parameters,
            testloader,
            device,
            num_classes=num_classes,
        ...
        if logger is not None:
            logger.log(
                ...
                test_acc=final_acc,
                invalid_answer_rate=final_invalid_rate,
        ...
        logger.write_summary(
            {
                ...
                "final_invalid_answer_rate": final_invalid_rate,
    ...
    for event in range(1, events + 1):
        ...
        if logger is not None:
            logger.log(
                ...
                invalid_answer_rate=final_invalid_rate if should_eval else "",
                num_examples=num_examples,
                delay=job.delay,
...
def _weighted_average(
    ...
    from fed_mllm.model import weighted_average
