...
class MMLUDataset(Dataset):
    def __init__(self, hf_dataset, tokenizer, max_length=512):
        ...
        self.labels = np.array([int(item["answer"]) for item in self.data])
...
def _dataset_labels(dataset: Dataset) -> np.ndarray:
    ...
    for idx in range(len(dataset)):
        item = dataset[idx]
        extracted.append(int(item["labels"]))
        # _image, label = item
...
def load_datasets(
    *,
    ...
    custom_data_dir = os.path.abspath("./data/mmlu")  
    hf_train = load_dataset("cais/mmlu", "all", split="auxiliary_train", cache_dir=custom_data_dir)
    hf_test = load_dataset("cais/mmlu", "all", split="test", cache_dir=custom_data_dir)
    
    train = MMLUDataset(hf_train, tokenizer)
    test = MMLUDataset(hf_test, tokenizer)
