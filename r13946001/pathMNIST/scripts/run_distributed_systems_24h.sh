...
enqueue_ablation() {
  ...
  enqueue_run "stage3_caa_v2_ablation" "$dataset" caa_fedbuff_v2 "$seed" resnet18 iid 0.5 heterogeneous_mild "ablation_${dataset}_full_caa_v2_s${seed}" "history_agreement_blend=0.25,client_fairness_power=0.5,delta_clip_multiplier=1.8" "full_caa_v2"
  enqueue_run "stage3_caa_v2_ablation" "$dataset" caa_fedbuff_v2 "$seed" resnet18 iid 0.5 heterogeneous_mild "ablation_${dataset}_no_server_ema_s${seed}" "history_agreement_blend=0.0,client_fairness_power=0.5,delta_clip_multiplier=1.8" "no_server_ema"
  enqueue_run "stage3_caa_v2_ablation" "$dataset" caa_fedbuff_v2 "$seed" resnet18 iid 0.5 heterogeneous_mild "ablation_${dataset}_no_fairness_s${seed}" "history_agreement_blend=0.25,client_fairness_power=0.0,delta_clip_multiplier=1.8" "no_fairness"
  enqueue_run "stage3_caa_v2_ablation" "$dataset" caa_fedbuff_v2 "$seed" resnet18 iid 0.5 heterogeneous_mild "ablation_${dataset}_no_clipping_s${seed}" "history_agreement_blend=0.25,client_fairness_power=0.5,delta_clip_multiplier=1000000.0" "no_clipping"
  signature="history_agreement_blend=0.25,client_fairness_power=0.5,adaptive_alpha_min=0.62,adaptive_alpha_max=0.62"
  enqueue_run "stage3_caa_v2_ablation" "$dataset" caa_fedbuff_v2 "$seed" resnet18 iid 0.5 heterogeneous_mild "ablation_${dataset}_static_alpha_s${seed}" "$signature" "static_alpha"
  enqueue_run "stage3_caa_v2_ablation" "$dataset" agreement_fedbuff_async "$seed" resnet18 iid 0.5 heterogeneous_mild "ablation_${dataset}_old_caa_s${seed}" "" "old_caa"
}
...
dist_path = Path("figures/report/distributed_systems_summary.csv")
if dist_path.exists():
    frame = pd.read_csv(dist_path)
    print(f"distributed_summary_rows={len(frame)}")
    if not frame.empty:
        print(f"non_iid_rows={(frame['partition'] == 'dirichlet').sum()}")
        stress = frame[frame["delay_label"].astype(str).str.contains("hetero|uniform|lognormal", na=False)]
        print(f"stress_rows={len(stress)}")
        ablation = frame[frame["variant"].astype(str).str.contains("full_caa_v2|no_server_ema|no_fairness|no_clipping|static_alpha|old_caa", na=False)]
        print(f"ablation_rows={len(ablation)}")
        async_frame = frame[frame["method"] != "sync_fedavg"]
        if not async_frame.empty:
            print(f"latest_avg_p95_staleness={async_frame['p95_staleness'].dropna().tail(20).mean():.4f}")
            print(f"latest_client_gini={async_frame['client_contribution_gini'].dropna().tail(20).mean():.4f}")
PY
}
...
generate_full_report_pack() {
  generate_report_pack
  python scripts/plot_classification_results.py --result-dir results --checkpoint-dir checkpoints --outdir figures/classification --datasets pathmnist pneumoniamnist bloodmnist organamnist breastmnist dermamnist >> "$ACTION_LOG" 2>&1 || true
}
